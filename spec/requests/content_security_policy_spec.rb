...
RSpec.describe 'Content-Security-Policy' do
  ...
  def expected_csp_headers
    <<~CSP.split("\n").map(&:strip)
      ...
      script-src 'self' #{local_domain} https://www.googletagmanager.com 'wasm-unsafe-eval' 'sha256-Z5KW83D+6/pygIQS3h9XDpF52xW3l3BHc7JL9tj3uMs=' 'nonce-ZbA+JmE7+bK8F5qvADZHuQ=='
