...
export default class Card extends PureComponent {
  ...
  render () {
    ...
    if (interactive) {
            ...
            {revealed ? (
              <div className='status-card__actions' onClick={this.handleEmbedClick} role='none'>
                <div>
                  ...
                  <a href={card.get('url')} onClick={this.handleExternalLinkClick} target='_top' rel='noopener'><Icon id='external-link' icon={OpenInNewIcon} /></a>
                </div>
              </div>
            ) : spoilerButton}
          </div>
      ...
      return (
        <div className={classNames('status-card', { expanded: largeImage })} ref={this.setRef} onClick={revealed ? null : this.handleReveal} role={revealed ? 'button' : null}>
          {embed}
          <a href={card.get('url')} target='_top' rel='noopener'>{description}</a>
        </div>
      );
    ...
    return (
      <>
        <a href={card.get('url')} className={classNames('status-card', { expanded: largeImage, bottomless: showAuthor })} target='_top' rel='noopener' ref={this.setRef}>
          {embed}
          {description}
        </a>
